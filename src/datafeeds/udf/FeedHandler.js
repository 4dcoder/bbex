import OHLCRequestResponseHandler from './OHLCRequestResponseHandler';
...
FeedHandler.prototype.init = function() {
  this._webSocketConnection = new ReconnectingWebSocket('ws://localhost:3001');
  ...
  */
      ...
      case 'tick':
        ...
        if (data.tick.error) {
          ...
          if (data.echo_req.tradingview_ticker_id) {
            var tradingview_ticker_id =
              TradingView.currentlyDisplayedSymbol + TradingView.actualResolution;
  ...
  setInterval(function() {
    ...
    if (timeElapsedAfterLastTick >= 60000) {
      //60 seconds
